...
import java.util.*;
  ...
  /*
   * Creates a king piece object with the specified team and position
   * 
  ...
  public List<int[]> possibleCaptures(Gameboard gameBoard) {
    ...
    if (position[0] < 6 && position[1] < 6 &&
        gameBoard.getPiece(new int[] { advanceRowPosition, position[1] + 1 }) != null && // check if diagonal to the
                                                                                         // bottom right isn't empty
        gameBoard.getPiece(new int[] { advanceRowPosition, position[1] + 1 }).getTeam().equals(opposingTeam) && //check if diagonal to bottom right is enemy
     
        gameBoard.getPiece(new int[] { advanceRowPosition + 1, position[1] + 2 }) == null) { // check to see if diagonal
                                                                                             // across is empty
    ...
    if (position[0] < 6 && position[1] > 1 &&
        gameBoard.getPiece(new int[] { advanceRowPosition, position[1] - 1 }) != null && // check if diagonal to the
                                                                                         // bottom left isn't empty
        gameBoard.getPiece(new int[] { advanceRowPosition, position[1] - 1 }).getTeam().equals(opposingTeam) && // check if diagonal to bottom left is enemy
    ...
    if (position[0] > 1 && position[1] < 6 &&
        gameBoard.getPiece(new int[] { advanceRowPosition, position[1] + 1 }) != null && // check if diagonal to the top
                                                                                         // right isn't empty
        gameBoard.getPiece(new int[] { advanceRowPosition, position[1] + 1 }).getTeam().equals(opposingTeam) && // check if top right is enemy
        gameBoard.getPiece(new int[] { advanceRowPosition - 1, position[1] + 2 }) == null) { // check to see if diagonal
                                                                                             // across is empty
      moves.add(new int[] { advanceRowPosition - 1, position[1] + 2 });
    ...
    if (position[0] > 1 && position[1] > 1 &&
        gameBoard.getPiece(new int[] { advanceRowPosition, position[1] - 1 }) != null && // check if diagonal to the top
                                                                                         // left isn't empty
        gameBoard.getPiece(new int[] { advanceRowPosition, position[1] - 1 }).getTeam().equals(opposingTeam) && // check if top left is enemy
