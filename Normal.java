/*
* Creates a Normal piece that can only move forward
  ...
  public List<int[]> possibleCaptures(Gameboard gameBoard) {
    ...
    if(team.equals("mouse")){
      ...
      if(position[0] < 6 && position[1] < 6 &&
        gameBoard.getPiece(new int[]{advanceRowPosition, position[1] + 1}) != null && //check if diagonal to the right isn't empty
        gameBoard.getPiece(new int[]{advanceRowPosition, position[1] + 1}).getTeam().equals("cat") && // if diagonal to the right isn't empty check to see if the piece is of the opposing team
      ...
      if(position[0] < 6 && position[1] > 1 &&
        gameBoard.getPiece(new int[]{advanceRowPosition, position[1] - 1}) != null && //check if diagonal to the left isn't empty
        gameBoard.getPiece(new int[]{advanceRowPosition, position[1] - 1}).getTeam().equals("cat") && // if diagonal to the left isn't empty check to see if the piece is of the opposing team
        gameBoard.getPiece(new int[]{advanceRowPosition + 1, position[1] - 2}) == null){ //check to see if diagonal across is empty 
        moves.add(new int[]{advanceRowPosition+1,position[1]-2});
      }
    }else{
      ...
      if(position[0] > 1 && position[1] < 6 &&
        gameBoard.getPiece(new int[]{advanceRowPosition, position[1] + 1}) != null && //check if diagonal to the right isn't empty
        gameBoard.getPiece(new int[]{advanceRowPosition, position[1] + 1}).getTeam().equals("mouse") && // if diagonal to the right isn't empty check to see if the piece is of the opposing team
        gameBoard.getPiece(new int[]{advanceRowPosition - 1, position[1] + 2}) == null){ //check to see if diagonal across is empty 
        moves.add(new int[]{advanceRowPosition-1,position[1]+2});
