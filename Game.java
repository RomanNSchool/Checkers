...
class Game {
  ...
  private HashMap<String, String> playerTeam = new HashMap<String, String>();
  private Gameboard gameBoard;
  /*
   * Creates a game object with the specified players
   * 
   * @param player1 - String type - name of first player
   * @param player2 - String type - name of second player
   */
  ...
  /*
   * Handles gameplay
   * @param playAi - boolean type - is the current game player vs ai
   */
  public void play(Boolean playAi) {
    ...
    do {
      ...
      // switch teams for next round if no captures have been made or last played
      // piece cannot capture any other pieces
      if (newTotalPieces == totalPieces
          || gameBoard.getLastPlayedPiece().possibleCaptures(gameBoard).isEmpty() == true) {
    ...
    System.out.println();
    System.out.println(playerTeam.get(winner) + " is the winner!");
    if(!(playerTeam.get(winner).equals("AI"))){
      highScore.addScore(playerTeam.get(winner));
    }
  }
  ...
  public String hasWon(Gameboard gameBoard) {
    ...
    if (noOfCatPieces == 0) {
      ...
      return "cat";
      //stalemates
    } else if (moveableCatPieces.isEmpty()) {
      return "mouse";
    } else if(moveableMousePieces.isEmpty()){
      return "cat";
    } else {
      return "";
    }
  ...
  public void round(String team) {
    ...
    do {
      ...
      if (userPositionInput.length() == 2
          && (userPositionInput.charAt(0) == 'a' || userPositionInput.charAt(0) == 'b'
              || userPositionInput.charAt(0) == 'c' || userPositionInput.charAt(0) == 'd'
              || userPositionInput.charAt(0) == 'e' || userPositionInput.charAt(0) == 'f'
              || userPositionInput.charAt(0) == 'g' || userPositionInput.charAt(0) == 'h')
          && (userPositionInput.charAt(1) == '1' || userPositionInput.charAt(1) == '2'
              || userPositionInput.charAt(1) == '3' || userPositionInput.charAt(1) == '4'
              || userPositionInput.charAt(1) == '5' || userPositionInput.charAt(1) == '6'
              || userPositionInput.charAt(1) == '7' || userPositionInput.charAt(1) == '8')) {
        piece = gameBoard.getPiece(playerInputToPos(userPositionInput));
        // ensure that piece exists, is of the same team as the player, and has valid
        // moves
        if (piece != null && piece.getTeam().equals(team) && !(piece.validMoves(gameBoard).isEmpty())) {
          ...
          // if team has pieces that are able to capture other pieces, ensure that the
          // user selected one of those pieces (force capture)
  ...
  public int[] playerInputToPos(String input) {
    ...
    // subtract one because the numbers on the board start at 1, then subtract it
    // from 7 because the order is reversed on the board
